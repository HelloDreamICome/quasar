<template>
  <div class="q-layout-padding" :class="dark ? 'bg-black text-white' : null">
    ...
    <q-date
      ...
      @calendar-change="ym => rangeMinYearMonth = (ym.year + (ym.month === 12 ? 1 : 0)) + '/' + (ym.month === 12 ? '01' : (ym.month < 9 ? '0' : '') + (ym.month + 1))"
    />
    <q-date
      ref="qDateRangeEnd"
      v-model="dateRange"
      ...
      @input="dateRangeInputFocus = null"
      @calendar-change="ym => rangeMaxYearMonth = (ym.year - (ym.month === 1 ? 1 : 0)) + '/' + (ym.month === 1 ? 12 : (ym.month < 11 ? '0' : '') + (ym.month - 1))"
    />
    </div>
  </div>
</template>
